...
if __name__ == "__main__":
    ...
    output_file = arguments["<output-txt>"] # "../../data/deepsea_selene_output.txt"
    ...
    for f in features:
        ...
        if 'H' == target[0] and str.isdigit(target[1]):
            only_histone_features.append(f)
        
    with open(data_file, 'r') as read_fh, \
            open(output_file, 'w+') as write_fh:
        for line in read_fh:
            cols = line.strip().split('\t')
            if cols[-1] not in only_histone_features:
                continue
